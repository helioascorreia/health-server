...
class Mutation:
    @strawberry.mutation
    async def create_measurement(self, info, input_: MeasurementCreateInput) -> Measurement:
    ...
    @strawberry.mutation
    async def update_measurement(self, info, input_: MeasurementUpdateInput) -> Measurement:
        db = info.context["request"].app.ctx.mongo_db
        cleaned_input = clean_input(input_)
        ...
        updated_result = await db.measurement.update_one({"_id": ObjectId(id_)}, {"$set": cleaned_input})
    ...
    @strawberry.mutation
    async def delete_measurement(self, info, id_: strawberry.ID) -> bool:
        db = info.context["request"].app.ctx.mongo_db
